...
class Process: Equatable {
    let id: Int
    private(set) var requiredTime: Int
    private(set) var priority: Int
    
    init(id: Int, requiredTime: Int, priority: Int) {
        self.id = id
...
for _ in 0..<T {
    ...
    scheduledProcess.reduceTime()
    scheduledProcess.decreasePriority()
    if scheduledProcess.requiredTime > 0 {
        readyQueue.push(process: scheduledProcess)
    }
